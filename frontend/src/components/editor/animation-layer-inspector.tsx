...
import { ANIMATION_PRESETS } from "@/lib/editor-types";
import { parseAnimationData, type AnimationPreset, type Layer } from "@/lib/api";

export function AnimationLayerInspector({ layer }: { layer: Layer }) {
  const patchLayer = useEditorStore((s) => s.patchLayer);
  const patchLayerData = useEditorStore((s) => s.patchLayerData);
  const deleteLayer = useEditorStore((s) => s.deleteLayer);
  const data = parseAnimationData(layer.data);
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="flex flex-col gap-1.5 text-xs">
        ...
        <Select
          value={data.preset}
          onValueChange={(v) => patchLayerData(layer.id, { preset: v as AnimationPreset })}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
